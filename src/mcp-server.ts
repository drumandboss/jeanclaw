...
async function httpPost(path: string, body: Record<string, unknown>): Promise<{ ok: boolean; status: number; data: unknown }> {
...
async function httpGet(path: string): Promise<{ ok: boolean; status: number; data: unknown }> {
  const res = await fetch(`${BASE_URL}${path}`)
  const data = await res.json().catch(() => null)
  return { ok: res.ok, status: res.status, data }
}
...
async function writeConfig(config: ConfigJson): Promise<void> {
...
server.tool(
  'send_message',
  'Send a message to any configured channel (telegram, imessage) proactively.',
  {
  ...
  async ({ channel, peer_id, text }) => {
    ...
    if (!result.ok) {
      return { content: [{ type: 'text' as const, text: `Failed to send message: HTTP ${result.status}` }], isError: true }
    }
    return { content: [{ type: 'text' as const, text: `Message sent to ${channel}:${peer_id}` }] }
  },
)
...
server.tool(
  'manage_cron',
  'Add, remove, or list cron jobs. After add/remove, you should restart JeanClaw with `pm2 restart jeanclaw`.',
  {
  ...
  async ({ action, id, schedule, timezone, prompt, deliver_to, session }) => {
    ...
    if (action === 'add') {
      if (!id || !schedule || !timezone || !prompt || !deliver_to) {
        return {
          content: [{ type: 'text' as const, text: 'Missing required fields: id, schedule, timezone, prompt, deliver_to' }],
          isError: true,
        }
      }
      const existingCrons = config.crons ?? []
      if (existingCrons.some((c) => c.id === id)) {
        return { content: [{ type: 'text' as const, text: `Cron job "${id}" already exists. Remove it first.` }], isError: true }
      }
      const newCron = {
        id,
        schedule,
    ...
    if (action === 'remove') {
      ...
      if (filtered.length === existingCrons.length) {
        return { content: [{ type: 'text' as const, text: `Cron job "${id}" not found.` }], isError: true }
      }
...
server.tool(
  'manage_heartbeat',
  'Enable/disable heartbeat or change interval.',
  {
  ...
  async ({ action, interval }) => {
    ...
    if (action === 'enable') {
      ...
      if (!interval) {
        return { content: [{ type: 'text' as const, text: 'Missing required field: interval' }], isError: true }
      }
      // Basic validation
      if (!/^\d+(h|m|s)$/.test(interval)) {
        return { content: [{ type: 'text' as const, text: `Invalid interval format: "${interval}". Use e.g. "2h", "30m", "45s".` }], isError: true }
      }
...
server.tool(
  'manage_identity',
  'Read or update identity files (SOUL.md, USER.md, etc).',
  {
  ...
  async ({ action, file, content }) => {
    ...
    if (!filename) {
      return { content: [{ type: 'text' as const, text: `Unknown file: ${file}` }], isError: true }
    }
    ...
    if (action === 'read') {
      try {
        ...
        if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
          return { content: [{ type: 'text' as const, text: `File ${filename} does not exist yet.` }] }
        }
        return { content: [{ type: 'text' as const, text: `Error reading ${filename}: ${(err as Error).message}` }], isError: true }
      }
    }

    if (action === 'update') {
      if (content === undefined || content === null) {
        return { content: [{ type: 'text' as const, text: 'Missing required field: content' }], isError: true }
      }
      try {
        await writeFile(filePath, content, 'utf-8')
        return { content: [{ type: 'text' as const, text: `${filename} updated successfully.` }] }
      } catch (err) {
        return { content: [{ type: 'text' as const, text: `Error writing ${filename}: ${(err as Error).message}` }], isError: true }
      }
    }
...
server.tool(
  'bot_status',
  'Get JeanClaw daemon status including sessions, config, and health.',
  {},
  async () => {
    try {
      const result = await httpGet('/api/status')
      if (!result.ok) {
        return { content: [{ type: 'text' as const, text: `Failed to get status: HTTP ${result.status}` }], isError: true }
      }
      return { content: [{ type: 'text' as const, text: JSON.stringify(result.data, null, 2) }] }
    } catch (err) {
      return {
        content: [{ type: 'text' as const, text: `Cannot reach daemon at ${BASE_URL}: ${(err as Error).message}` }],
        isError: true,
      }
    }
  },
...
server.tool(
  'generate_image',
  'Generate an image using DALL-E. Returns the file path of the generated image.',
  {
  ...
  async ({ prompt: imagePrompt }) => {
    ...
    if (!apiKey) {
      return {
        content: [{ type: 'text' as const, text: 'Image generation not configured. Set OPENAI_API_KEY environment variable.' }],
        isError: true,
      }
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return { content: [{ type: 'text' as const, text: `OpenAI API error (${res.status}): ${errBody}` }], isError: true }
      }
      ...
      return { content: [{ type: 'text' as const, text: `Image generated and saved to: ${filePath}` }] }
    } catch (err) {
      return { content: [{ type: 'text' as const, text: `Image generation failed: ${(err as Error).message}` }], isError: true }
    }
  },
)
...
server.tool(
  'text_to_speech',
  'Convert text to speech audio using OpenAI TTS. Returns the file path of the audio.',
  {
  ...
  async ({ text: ttsText }) => {
    ...
    if (!apiKey) {
      return {
        content: [{ type: 'text' as const, text: 'TTS not configured. Set OPENAI_API_KEY environment variable.' }],
        isError: true,
      }
    }
    ...
    try {
      ...
      if (!res.ok) {
        const errBody = await res.text()
        return { content: [{ type: 'text' as const, text: `OpenAI TTS API error (${res.status}): ${errBody}` }], isError: true }
      }
      ...
      return { content: [{ type: 'text' as const, text: `Audio generated and saved to: ${filePath}` }] }
    } catch (err) {
      return { content: [{ type: 'text' as const, text: `TTS failed: ${(err as Error).message}` }], isError: true }
    }
  },
)
