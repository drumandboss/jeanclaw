...
export class Daemon {
    ...
    static ERROR_COOLDOWN_MS = 60_000;
    async start(configPath) {
        ...
        if (this.config.channels.telegram.enabled) {
            ...
            // Wire slash commands
            tgAdapter.onCommand(async (channelKey, command) => {
        ...
        this.scheduler = new Scheduler({
            ...
            onHeartbeat: async () => {
                ...
                try {
                    const heartbeatContent = await readFile(join(this.config.workspace, 'HEARTBEAT.md'), 'utf-8');
                    ...
                    log.error('heartbeat execution failed', { error: err.message });
                }
            },
            onCron: async (job) => {
                log.info('cron job triggered', { id: job.id });
                try {
                    const session = await this.sessionManager.getOrCreate(job.session === 'isolated' ? `__cron_${job.id}__` : '__cron_shared__');
                    const responsePromise = new Promise((resolve) => {
                        let text = '';
                        const onEvent = (event) => {
                            if (event.type === 'assistant' && event.message)
                    ...
                    log.error('cron execution failed', { id: job.id, error: err.message });
                }
            },
        });
    ...
    async processMessage(entry) {
        ...
        try {
                    ...
                    if (event.type === 'assistant_partial' && event.message) {
                        ...
                        if (partial) {
                            ...
                            this.maybeStreamUpdate(channelName, peerId, fullResponse, sentMessageId, lastStreamEdit, STREAM_INTERVAL)
                                ...
                                if (result.editTime)
                                    lastStreamEdit = result.editTime;
                            })
                                .catch(() => { });
                        }
                    }
                ...
                if (sentMessageId) {
                    ...
                    try {
                        ...
                        if (adapter && 'bot' in adapter) {
                            ...
                            try {
                                await adapter.bot.api.editMessageText(chatId, sentMessageId, html, { parse_mode: 'HTML' });
                            }
                            catch {
                                try {
                                    await adapter.bot.api.editMessageText(chatId, sentMessageId, chunks[0]);
                                }
                                catch { /* ignore */ }
                            }
            ...
            log.error('message handling failed', { channelKey, error: err.message });
            await this.sendErrorIfAllowed(channelKey, peerId, err);
        }
        finally {
    ...
    async sendErrorIfAllowed(channelKey, peerId, err) {
        const now = Date.now();
    ...
    sendTypingIndicator(channelName, peerId) {
        try {
            const adapter = this.router.getAdapter(channelName);
            if (adapter && 'bot' in adapter) {
                const chatId = parseInt(peerId, 10);
                adapter.bot.api.sendChatAction(chatId, 'typing').catch(() => { });
            }
        }
        catch { /* ignore typing errors */ }
    }
    async maybeStreamUpdate(channelName, peerId, text, sentMessageId, lastEditTime, interval) {
        const now = Date.now();
        const adapter = this.router.getAdapter(channelName);
        if (!adapter || !('bot' in adapter)) {
        ...
        if (!sentMessageId) {
            ...
            try {
                const sent = await adapter.bot.api.sendMessage(chatId, displayText);
        ...
        if (now - lastEditTime >= interval) {
            try {
                await adapter.bot.api.editMessageText(chatId, sentMessageId, displayText);
    ...
    async handleCommand(channelKey, command) {
        ...
        this.dailyResetTimer = setInterval(async () => {
            ...
            if (now.getHours() === 4 && now.getMinutes() === 0) {
                ...
                try {
                    ...
                    log.error('daily reset failed', { error: err.message });
                }
            }
        }, 60_000);
    }
    async deliverToDefaultChannel(text) {
