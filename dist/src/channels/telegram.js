 ...
 */
export function splitForTelegram(text, maxLen = MAX_MESSAGE_LENGTH) {
...
export class TelegramAdapter {
    ...
    async start() {
        ...
        this.bot.on('message:photo', async (ctx) => {
            ...
            try {
                ...
                log.error('photo handling failed', { error: err.message });
        ...
        this.bot.on('message:voice', async (ctx) => {
            ...
            try {
                ...
                log.error('voice handling failed', { error: err.message });
    ...
    async sendStreaming(peerId, chunks) {
        const chatId = parseInt(peerId, 10);
        let sentMsg = null;
        let fullText = '';
        let lastEdit = 0;
    ...
    async handleCommand(ctx, userId, command) {
        ...
        try {
            ...
            log.error('command handler failed', { command, error: err.message });
    ...
    async sendSingleMessage(chatId, text) {
        ...
        try {
            ...
            try {
                ...
                log.error('failed to send message', { chatId, error: err2.message });
            }
        }
    }
    async sendEditOrFallback(chatId, messageId, text) {
        const html = markdownToTelegramHtml(text);
        try {
